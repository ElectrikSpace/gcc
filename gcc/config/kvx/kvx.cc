...
kvx_print_operand (FILE *file, rtx x, int code)
{
  ...
  switch (GET_CODE (operand))
	...
	if (GET_MODE (x) == HFmode)
	  ...
	  {
	    REAL_VALUE_TO_TARGET_SINGLE (r, l[0]);	/* FIXME */
	    fprintf (file, "0x%08x", (unsigned int) l[0]);
	    return;
	  }
	else if (GET_MODE (x) == SCmode)
	  {
	    REAL_VALUE_TO_TARGET_DOUBLE (r, l);	/* FIXME */
	    fprintf (file, "0x%08x%08x", (unsigned int) l[1],
		     (unsigned int) l[0]);
	    return;
	  }
...
kvx_const_vector_value (rtx x, int slice)
{
  ...
  if (GET_CODE (x) == CONST_VECTOR)
	...
	value = (val_0_r & 0xFF) | (val_0_i & 0xFF) << 8 | (val_1_r & 0xFF) << 16
		| (val_1_i & 0xFF) << 24 | (val_2_r & 0xFF) << 32
		| (val_2_i & 0xFF) << 40 | (val_3_r & 0xFF) << 48
		| (val_3_i & 0xFF) << 56;
      }
      else
      {
       ...
       else if (inner_mode == CHImode)
	{
      if (CONST_VECTOR_P (CONST_VECTOR_ELT (x, 0)))
      {
	HOST_WIDE_INT val_0_r = INTVAL (CONST_VECTOR_ELT (CONST_VECTOR_ELT (x, index + 0), 0));
	HOST_WIDE_INT val_0_i = INTVAL (CONST_VECTOR_ELT (CONST_VECTOR_ELT (x, index + 0), 1));
	HOST_WIDE_INT val_1_r = INTVAL (CONST_VECTOR_ELT (CONST_VECTOR_ELT (x, index + 1), 0));
	HOST_WIDE_INT val_1_i = INTVAL (CONST_VECTOR_ELT (CONST_VECTOR_ELT (x, index + 1), 1));
	value = (val_0_r & 0xFFFF) | (val_0_i & 0xFFFF) << 16
		| (val_1_r & 0xFFFF) << 32 | (val_1_i & 0xFFFF) << 48;
      }
      else
      {
	HOST_WIDE_INT val_0 = INTVAL (CONST_VECTOR_ELT (x, index + 0));
